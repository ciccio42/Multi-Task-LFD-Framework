...
# for MODEL in $BASE_PATH/mosaic-baseline-sav-folder/baseline-1/1Task-Pick-Place-Balanced-Dataset-RMSProp-No-Strong-Aug-No-Crop-Twice-RGB-Batch64-1gpu-Attn2ly128-Act2ly256mix4-headCat-simclr128x512
...
# python3 $BASE_PATH/repo/mosaic/tasks/test_models/test_any_task.py $MODEL --env $TASK --saved_step $S --eval_each_task 10 --num_workers 1 --project_name 'mosaic_balanced_batch_rmsprop_rgb' --debug
# done

# done
# done

for MODEL in $BASE_PATH/mosaic-baseline-sav-folder/baseline-1/1Task-Pick-Place-Balanced-Dataset-Adam-No-Strong-Aug-No-Crop-Twice-RGB-lr-schedule-Batch64-1gpu-Attn2ly128-Act2ly256mix4-actCat-simclr128x512

do
for S in 200475 232875 
do
for TASK in pick_place
do

# # python3 $BASE_PATH/repo/mosaic/tasks/test_models/test_any_task.py $MODEL --wandb_log --env $TASK --saved_step $S --eval_each_task 10 --num_workers 5 --project_name 'mosaic_baseline_1_stable_policy'
python3 $BASE_PATH/repo/mosaic/tasks/test_models/test_any_task.py $MODEL --env $TASK --saved_step $S --eval_each_task 10 --num_workers 5 --project_name 'mosaic_balanced_batch_adam_rgb' --wandb_log
done
done
done

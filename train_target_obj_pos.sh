...
PROJECT_NAME=target_pose_classification_10_random_frames
python repo/mosaic/train_target_obj_pos.py --config-path ${CONFIG_PATH} --config-name ${CONFIG_NAME} policy='${mosaic}' task_names=${TASK_str} exp_name=${EXP_NAME} bsize=${BSIZE} vsize=${BSIZE} epochs=${EPOCH} debug=false wandb_log=true resume=false project_name=${PROJECT_NAME}

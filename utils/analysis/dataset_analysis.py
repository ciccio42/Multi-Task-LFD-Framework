...
def single_run(model, agent_env, context, img_formatter, variation, show_image, agent_trj, indx):
    ...
    with torch.no_grad():
        start.record()
        traj, info, context = eval_fn(model=model, env=agent_env, context=context, gpu_id=0, variation_id=variation,
...
def run_inference(model, conf_file, task_name, task_indx, results_dir_path, training_trj, show_image, experiment_number, file_pair):
    ...
    for i in range(cnt):
        ...
        traj, info = single_run(model=model, agent_env=agent_env, context=context, img_formatter=img_formatter,

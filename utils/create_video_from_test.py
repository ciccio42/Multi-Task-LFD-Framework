...
for step_path in glob.glob(step_pattern)[:1]:
    ...
    for context_file, traj_file in zip(context_files, traj_files):
        ...
        out_path = f"{task_name}_step_{step}_demo{context_file[-5:-4]}_traj{traj_file[-5:-4]}.mp4"
        out = cv2.VideoWriter(os.path.join(video_path,out_path), fourcc, 25.0, (output_width, output_height))

...
# some settings
DELTA_POS_KEY_PRESS = 0.01      # delta camera position per key press
DELTA_ROT_KEY_PRESS = 1         # delta camera angle per key press

def get_exp_env(env_type, controller_type, renderer=False, camera_obs=True, task=None, ret_env=False, seed=None, env_seed=None, gpu_id=0, render_camera="frontview",**kwargs):
...
def get_target_pose(delta_pos, base_pos, quat):
...
if __name__ == '__main__':
    ...
    if len(inp) == 0:
        inp = '<camera mode="fixed" name="frontview" pos="0.0 0.0 1.45" quat="0.56 0.43 0.43 0.56"/>'
    ...
    controller_config_path = os.path.join(current_dir,"osc_pose.json")
    controller_config = load_controller_config(custom_fpath=controller_config_path)
    env = get_exp_env('UR5e_PickPlaceDistractor', 
    ...
    print(np.reshape(env.sim.data.site_xmat[env.robots[0].eef_site_id], (3,3)))
    while True:
        action = np.concatenate((get_target_pose(delta_pos=[0.0,0.0,0.0], 
                                                base_pos=env.sim.data.site_xpos[env.robots[0].eef_site_id],
                                                quat=Quaternion(matrix=np.reshape(env.sim.data.site_xmat[env.robots[0].eef_site_id], (3,3)))),
                                                [-1]))
